...
data_path = "/scratch/ssd004/scratch/zjt/yifei/"

def load_data(tasks, data_path):
    '''
    Load queries and remove queries not in tasks
    '''
    test_queries = pickle.load(open(os.path.join(data_path, "test-queries.pkl"), 'rb'))
    
    test_hard_answers = pickle.load(open(os.path.join(data_path, "test-easy-answers.pkl"), 'rb'))
    test_easy_answers = pickle.load(open(os.path.join(data_path, "test-hard-answers.pkl"), 'rb'))
...
def load_info(data_path):
    ...
    with open(os.path.join(data_path, "entity2text.txt"), "r") as f:
...
def hop_query_to_text(alignment_data, id2ent, id2rel, ent2text, query):
    ...
    for relation in query[1]:
        ...
        if "{head}" not in text_relation or "{tail}" not in text_relation:
            return ""
    ...
    for rel in relations:
        if not head_used:
            num_intermediate += 1
            formatted_rel = rel.format(head=head_entity, tail=f"X_{num_intermediate}")
            final_question += " "
            final_question += formatted_rel

            head_used = True
        else:
            formatted_rel = rel.format(head=f"X_{num_intermediate}", tail=f"X_{num_intermediate+1}")
            num_intermediate += 1
            final_question += " "
            final_question += formatted_rel

    final_question = final_question.replace(f"X_{num_intermediate}", "[Y]")
...
if __name__ == "__main__":
    ...
    test_queries, test_hard_answers, test_easy_answers = load_data("2p 3p", data_path)
